...
def test_ruido_blanco_da_espectro_plano(rng):
    ...
    assert np.all(psd > 0)
    assert abs(np.std(psd) / media_psd - 1.0 / np.sqrt(len(psd))) < 0.5  # Comprobación de estabilidad estadística
...
def test_parseval(rng):
    ...
    fs = 1000.0
    nperseg = 256
    senal = rng.normal(2.0, 3.5, 10000)  # Media 2.0, desviación 3.5 -> Varianza ~ 3.5^2 = 12.25
