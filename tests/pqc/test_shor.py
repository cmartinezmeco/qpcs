...
import numpy as np
from pqc.shor import medir_fase_15

# tests/pqc/test_shor.py (parte 2.3)
from math import gcd
from pqc.shor import factorizar_15, orden_desde_fase

def test_fase_es_multiplo_de_1_sobre_4_para_a7():
    """a=7 mod 15 tiene orden 4: las fases validas son 0, 1/4, 2/4, 3/4."""
    # Con varias semillas, la fase medida cae cerca de algun k/4.
    for seed in range(8):
        phi = medir_fase_15(a=7, n_count=8, seed=seed)
...
def test_reduccion_es_correcta_para_todo_a():
    ...
    for a in (2, 4, 7, 8, 11, 13):
        r = next(k for k in range(1, N) if pow(a, k, N) == 1) # orden real
        if r % 2 == 0 and pow(a, r // 2, N) != N - 1:
            x = pow(a, r // 2, N)
            assert 1 < gcd(x - 1, N) < N or 1 < gcd(x + 1, N) < N

def test_fracciones_continuas_recuperan_denominador():
    """phi = 3/4 debe dar r = 4 (denominador del convergente)."""
    assert orden_desde_fase(0.75, N=15) == 4